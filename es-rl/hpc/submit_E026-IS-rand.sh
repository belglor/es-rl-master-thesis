...
ID="E026-IS"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --optimize-sigma None --env-name MNIST --model MNISTNet --optimizer SGD --momentum 0.9 --lr 0.05 --cov-lr 0.05 --max-generations 1000 --batch-size 1000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
                    "$COMMON_IN --forced-refresh 0"
					"$COMMON_IN --forced-refresh 1"
					"$COMMON_IN --forced-refresh 5"
					"$COMMON_IN --forced-refresh 10"
                    "$COMMON_IN --forced-refresh 20"
                    "$COMMON_IN --forced-refresh 30"
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
