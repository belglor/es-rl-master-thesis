...
ID="E031-NORM"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --env-name MNIST --model MNISTNet --perturbations 100 --optimizer SGD --momentum 0.9 --lr 0.05 --max-generations 3000 --batch-size 1000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --optimize-sigma None"						# Vanilla ES
					"$COMMON_IN --optimize-sigma single --cov-lr 2"		    # VO Isotropic
					"$COMMON_IN --optimize-sigma per-layer --cov-lr 4"	    # VO Seperable
					"$COMMON_IN --optimize-sigma per-weight --cov-lr 4"	    # VO Separable
				   )
				   # Uses momentum 0.9 and dampening 0.9 on beta
SCRIPT="run_hpc.sh"
REPEATS=2
