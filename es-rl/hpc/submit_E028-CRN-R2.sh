...
ID="E028-CRN-R2"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --perturbations 40 --optimize-sigma None --env-name Seaquest-v0 --model DQN --optimizer SGD --momentum 0.9 --lr 0.02 --cov-lr 0.02 --max-generations 250 --batch-size 10000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --common-random-numbers"
					"$COMMON_IN"
                    "$COMMON_IN --no-antithetic"
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
