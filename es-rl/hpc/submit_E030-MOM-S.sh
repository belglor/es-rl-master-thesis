...
ID="E030-MOM-S"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --env-name MNIST --model MNISTNet --perturbations 100 --optimizer SGD --lr 0.05 --optimize-sigma None --max-generations 1000 --batch-size 1000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --momentum 0.0"
					"$COMMON_IN --momentum 0.1"
					"$COMMON_IN --momentum 0.9"
					"$COMMON_IN --momentum 0.99"
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
