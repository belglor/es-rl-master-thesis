...
ID="E010"
COMMON_IN="--id ${ID} --env-name MNIST --optimizer SGD --lr-scheduler ExponentialLR --lr 0.2 --cov-lr 2 --max-generations 10000 --batch-size 1000 --perturbations 100 --safe-mutation SUM --chkpt-int 3600"
declare -a INPUTS=(
					"$COMMON_IN --algorithm NES --gamma 0.99977 --optimize-sigma None"
					"$COMMON_IN --algorithm NES --gamma 0.99977 --optimize-sigma single"
					"$COMMON_IN --algorithm sNES --gamma 0.99977 --optimize-sigma None"
					"$COMMON_IN --algorithm sNES --gamma 0.99977 --optimize-sigma single"
					"$COMMON_IN --algorithm sNES --gamma 0.99977 --optimize-sigma per-layer"
					"$COMMON_IN --algorithm ES --gamma 0.99977 --optimize-sigma None"
					"$COMMON_IN --algorithm ES --gamma 0.99977 --optimize-sigma single"
					"$COMMON_IN --algorithm sES --gamma 0.99977 --optimize-sigma None"
					"$COMMON_IN --algorithm sES --gamma 0.99977 --optimize-sigma single"
					"$COMMON_IN --algorithm sES --gamma 0.99977 --optimize-sigma per-layer"
				   )
SCRIPT="run_hpc.sh"
REPEATS=10
