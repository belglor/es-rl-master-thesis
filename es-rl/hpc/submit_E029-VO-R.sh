...
ID="E029-VO-R"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --perturbations 100 --env-name CartPole-v1 --model ClassicalControlFNN --optimizer SGD --momentum 0.9 --lr 0.05 --max-generations 500 --batch-size 10000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --optimize-sigma None"
					"$COMMON_IN --optimize-sigma single --cov-lr 0.005"
                    "$COMMON_IN --optimize-sigma per-layer --cov-lr 0.005"
                    "$COMMON_IN --optimize-sigma per-weight --cov-lr 0.005"
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
