...
ID="E022-AS"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --optimize-sigma single --env-name MNIST --model MNISTNet --optimizer SGD --momentum 0.9 --lr 0.05 --cov-lr 0.05 --max-generations 1000 --batch-size 1000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --perturbations 50 --no-antithetic"
					"$COMMON_IN --perturbations 100 --no-antithetic"
					"$COMMON_IN --perturbations 100"
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
