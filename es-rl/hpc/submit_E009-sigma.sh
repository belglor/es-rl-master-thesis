...
ID="E009"
COMMON_IN="--id ${ID} --env-name MNIST --optimizer SGD --lr-scheduler ExponentialLR --lr 0.050 --max-generations 10000 --batch-size 1000 --perturbations 100"
declare -a INPUTS=(
				   "$COMMON_IN --algorithm NES --gamma 0.99970 --optimize-sigma single"
				   "$COMMON_IN --algorithm NES --gamma 1 --optimize-sigma single"
				   "$COMMON_IN --algorithm NES --gamma 0.99970"
				   "$COMMON_IN --algorithm NES --gamma 1"
				   "$COMMON_IN --algorithm ES --gamma 0.99970 --optimize-sigma single"
				   "$COMMON_IN --algorithm ES --gamma 1 --optimize-sigma single"
				   "$COMMON_IN --algorithm ES --gamma 0.99970"
				   "$COMMON_IN --algorithm ES --gamma 1"
				   )
SCRIPT="run_hpc.sh"
REPEATS=10
...
# Monitorer
TOTAL_TIME="120:00"
MONITORER_INPUTS="-i $ID -t 600 -c -s"
