...
ID="E029-VO-S"
COMMON_IN="--id ${ID} --workers ${CORES} --algorithm sNES --env-name MNIST --model MNISTNet --perturbations 100 --optimizer SGD --momentum 0.9 --lr 0.05 --max-generations 1000 --batch-size 1000 --safe-mutation SUM --chkpt-int 1200 --lr-scheduler ExponentialLR --gamma 1"
declare -a INPUTS=(
					"$COMMON_IN --optimize-sigma None"						# Vanilla ES
					"$COMMON_IN --optimize-sigma single --cov-lr 1"		    # VO Isotropic
					"$COMMON_IN --optimize-sigma per-layer --cov-lr 1"	    # VO Seperable
					"$COMMON_IN --optimize-sigma per-weight --cov-lr 1"	    # VO Separable
				   )
SCRIPT="run_hpc.sh"
REPEATS=30
