...
ID="CartPole_per-weight_m0"
COMMON_IN="--id ${ID} --algorithm sNES --optimizer SGD --lr-scheduler ExponentialLR --gamma 1 --env-name CartPole-v0 --model ClassicalControlFNN --batch-size 1000 --safe-mutation None --optimize-sigma per-weight --lr 1"
declare -a INPUTS=(
				   "$COMMON_IN "
				   "$COMMON_IN --use-naturgrad --lr 20"
				   "$COMMON_IN --baseline_mu --baseline_sigma"				 
				   "$COMMON_IN --baseline_mu --baseline_sigma --use-naturgrad --lr 20"				   
				   )
SCRIPT="run_hpc.sh"
REPEATS=1
