...
ID="E011-Seaquest_lr_moregen"
COMMON_IN="--id ${ID} --algorithm sNES --optimizer SGD --lr-scheduler ExponentialLR --gamma 1 --env-name Seaquest-v0 --model DQN --batch-size 1000 --safe-mutation None --max-generations 500 --use-naturgrad"
declare -a INPUTS=(
				   "$COMMON_IN --optimize-sigma None"
				   "$COMMON_IN --optimize-sigma single"
				   "$COMMON_IN --optimize-sigma per-layer"
				   "$COMMON_IN --optimize-sigma per-weight"
				   "$COMMON_IN --optimize-sigma None --baseline_mu --baseline_sigma"
