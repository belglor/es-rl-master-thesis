...
ID="E007-Seaquest_submit_test"
COMMON_IN="--id ${ID} --algorithm sNES --optimizer SGD --lr-scheduler ExponentialLR --gamma 1 --env-name Seaquest-v0 --model DQN --batch-size 1000 --safe-mutation None"
declare -a INPUTS=(
				   "$COMMON_IN"
				   "$COMMON_IN --baseline_mu"
				   ...
				   "$COMMON_IN --optimize-sigma per-layer"
				   "$COMMON_IN --optimize-sigma per-layer --baseline_mu --baseline_sigma"
				   "$COMMON_IN --optimize-sigma per-layer --baseline_mu --baseline_sigma --use-naturgrad"
				   "$COMMON_IN --optimize-sigma per-layer --baseline_mu --baseline_sigma --use-naturgrad --lr 1"
				   "$COMMON_IN --optimize-sigma per-weight"
				   "$COMMON_IN --optimize-sigma per-weight --baseline_mu --baseline_sigma"
				   "$COMMON_IN --optimize-sigma per-weight --baseline_mu --baseline_sigma --use-naturgrad"
				   "$COMMON_IN --optimize-sigma per-weight --baseline_mu --baseline_sigma --use-naturgrad --lr 1"
				   )
SCRIPT="run_hpc.sh"
REPEATS=1
