...
def analyze(experiment_id, keys_to_plot):
    ...
    if stats:
        invert_signs(stats)
        create_plots(stats, keys_to_plot, 'generations', groups, result_dir, include_val=experiment_id == 'E027-CRN-S')
...
if __name__ == '__main__':
    ...
    # Experiment IDs
    experiment_ids = ['E027-CRN-S', 'E028-CRN-R1', 'E028-CRN-R2']
    # Keys to analyze
    keys_to_plot = [['return_unp', 'return_avg', 'accuracy_unp', 'accuracy_avg'],
                    ['return_unp', 'return_avg'],
                    ['return_unp', 'return_avg']]
    # Analyze
    for keys, experiment_id in zip(keys_to_plot, experiment_ids):
        analyze(experiment_id, keys)
