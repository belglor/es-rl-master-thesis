        ...
        try:
            ...
            if 'Use MU baseline       True' in s:
                if 'Use natural gradient  True' in s:
                    groups = np.append(groups, 'Baseline_mu_naturgrad' + optimizer)
                else:
                    groups = np.append(groups, 'Baseline_mu' + optimizer)
            else:
                groups = np.append(groups, 'Vanilla' + optimizer)
            stats.append(st)                
...
if __name__ == '__main__':
    ...
    # Experiment IDs
    experiment_ids = ['E008-Seaquest_no_sigma']
    # Optimizer labels
    # optimizers = [', SGD', ', ADAM']
    optimizers = ['', '']
    # Keys to analyze
